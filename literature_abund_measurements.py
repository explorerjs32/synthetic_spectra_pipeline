...
def residual(fit_params, obs_spec, stellar_params, model_params, model, element, atomic_number, lines_out, linelist_dir, temp_dir):
    ...
    #plt.plot(obs_spec['Wave'], obs_spec['Flux'], 'r-')
    #plt.plot(synth_spec['Wave'], synth_spec['Flux'], 'k-')
    #plt.show()

    # Resample the spectrum
    resamp_spec = spec_resampler(obs_spec, synth_spec)

    #plt.plot(resamp_spec['Wave'], resamp_spec['Flux'], 'r-')
    #plt.plot(synth_spec['Wave'], synth_spec['Flux'], 'k-')
    #plt.show()
    ...
    #plt.plot(masked_obs_spec['Wave'], masked_obs_spec['Flux'], 'r-')
    #plt.plot(masked_synth_spec['Wave'], masked_synth_spec['Flux'], 'k-')
    #plt.show()
...
literature_parameters_df = pd.read_csv('../literature_tables/benchmark_stars_literature_parameters.txt', sep='\s+', \
...
obs_spec = pd.read_csv(observed_spectrum_dir+star+'_spectrum.dat', delimiter='\s+', names=['Wave','Flux','Intensity'], engine='python')
...
literature_star_linelist = pd.read_csv('../literature_tables/'+star+'_linelist.csv', sep='\s+').fillna(0.1)
...
model_params, model_out = generate_marcs_model(star, star_parameters)
...
for fit_params in tqdm(abundances):
    ...
    out = residual(fit_params, obs_spec, star_parameters, model_params, model_out, element, atomic_num, lines_out, linelist_dir, temp_dir)
...
results_out.to_csv(save_dir+star+'_'+element+'_abundances_v3_1.csv', index=False)

#!/bin/bash
#SBATCH --job-name=abundance_syntspec_lib                   # Job name
...
#SBATCH --output=../log_files/abund_lib_out_%A_%a.out       # Standard output log
#SBATCH --error ../log_files/abund_lib_error_%A_%a.error    # Standard error log
...
# Define the star, input index, and element
star="$1"
element=$2
input_idx=$3

# Create the copies for all the necesary files
scp /blue/rezzeddine/share/fmendez/pipeline/abundance_syntspec_library_create.py /blue/rezzeddine/share/fmendez/pipeline/abundance_syntspec_library_create_${input_idx}_${SLURM_ARRAY_TASK_ID}.py
...
chmod a+x /blue/rezzeddine/share/fmendez/pipeline/spectra_smooth_${input_idx}_${SLURM_ARRAY_TASK_ID}.sh
...
echo $input_idx $element $sol_abundace
...
echo Running abundance_with_uncertainties_${input_idx}_${SLURM_ARRAY_TASK_ID}.py for ${star} ${element} using a ${abund_arr[$index]} abundance value

python3 /blue/rezzeddine/share/fmendez/pipeline/abundance_syntspec_library_create_${input_idx}_${SLURM_ARRAY_TASK_ID}.py -tsbs ${ts_bash} -tss ${ts_script} -a ${abundance} -bbs ${broadening_bash} -ibs ${inst_broad_script_out} -rbs ${rot_vel_broad_script_out} -s ${star} -e ${element} &
...
rm /blue/rezzeddine/share/fmendez/pipeline/abundance_syntspec_library_create_${input_idx}_${SLURM_ARRAY_TASK_ID}.py

...
model_list = sorted(os.listdir(models_dir))

for index, row in model_df.iterrows():
    # Define the model name
    mname = 'model_'+str(row['Temp'])+'g'+str(row['logg'])+'z'+str(row['z'])+'vt'+str(row['vt'])+'.TURBO'

    if mname in model_list:
        print(f'{mname} found in {index}')
        model_df = model_df.drop(labels=index, axis=0)

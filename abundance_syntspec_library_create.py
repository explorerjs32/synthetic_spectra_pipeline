...
parser.add_argument('-tsbs', '--tsbs', '--turbo_spectrum_bash_script', default='abund_spec_create.sh', type=str, help='Bash script to run TurboSpectrum script to generate synthetic spectra')
...
model_params, model_out = generate_marcs_model(args.s, star_parameters.iloc[0])

# Generate the synthetic spectra
synth_spec_out = generate_synthetic_spectra(model_params, star_parameters.iloc[0], round(args.a, 2), model_out, args.e, atomic_num, args.lld, args.tsbs, args.tss, args.tod)

# Apply instrumental and rotation broadening
final_synth_spec = spec_broadening(args.s, star_parameters.iloc[0], synth_spec_out, args.tod, args.bbs, args.ibs, args.rbs)